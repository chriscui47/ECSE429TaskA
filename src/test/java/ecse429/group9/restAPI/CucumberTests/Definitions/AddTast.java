...
public class AddTast {
    ...
    public void thereExistsATodoListInTheSystemWithTitle(String projectTitle) throws Throwable {
        ...
        try {
            APIInstance.post(validID,projectJson.toString());
        } catch (IOException e) {
            error = true;
        }
    ...
    public void iAddANewTaskWithTheTitleToTheTodoList(String taskTitle, String projectTitle) throws Throwable {
        ...
        try {
            APIInstance.post(validID, todoJson.toString());
        } catch (IOException e) {
            error = true;
        }
        try {
        ...
        JSONObject pResponse = APIInstance.send("GET", "/projects?title=" + projectTitle);
        String projectID = pResponse.getJSONArray("projects").getJSONObject(0).getString("id");

        //Get to do ID from its title
        JSONObject tResponse = APIInstance.send("GET", "/todos?title=" + taskTitle);
        if (tResponse == null){
        ...
        try {
            APIInstance.post("/projects/"+projectID+"/tasks",relationJson.toString());
        } catch (IOException e) {
            error = true;
        }
        try {
    ...
    public void thereWillBeANewTaskWithTitleInTheTodoList(String taskTitle, String projectTitle) throws Throwable {
        ...
        JSONObject response = APIInstance.send("GET","/projects/"+projectID+"/tasks");
    ...
    public void iAddANewTaskWithTheTitleToTheWrongTodoList(String taskTitle, String wrongTitle) {
        ...
        //Get project ID from its title
        try{
            pResponse = APIInstance.send("GET", "/projects?title=" + wrongTitle);
        }  catch (IOException e) {
            error = true;
        }
        try {
